...
def speak(text):
    try:
        ...
        pass  # Ignore error if the loop is already running
    ...
    while True:
        if listening:
            ...
            if "how are you" in query:
                ...
                speak("On what email address do you want to send it, sir? Please enter it in the terminal.")
                receiver_address = input("Email address: ")
                speak("What should be the subject, sir?")
                subject = take_command().capitalize()
                speak("What is the message?")
                message = take_command().capitalize()
                ...
                speak("I am reading out the latest headline for you, sir.")
                headlines = get_news()
                speak(headlines)
                print(*headlines, sep='\n')
                ...
                weather, temp, feels_like = weather_forecast(city)
                speak(f"The current temperature is {temp}, but it feels like {feels_like}.")
                speak(f"Also, the weather report mentions {weather}.")
                print(f"Description: {weather}\nTemperature: {temp}\nFeels like: {feels_like}")
